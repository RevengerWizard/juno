...
#ifndef _MSC_VER
   #if __GNUC__
      #define ___forceinline inline
   #else
      #define ___forceinline
   #endif
#endif
...
static ___forceinline uint32 crc32_update(uint32 crc, uint8 byte)
{
...
static ___forceinline void prep_huffman(vorb *f)
{
   if (f->valid_bits <= 24) {
      if (f->valid_bits == 0) f->acc = 0;
      do {
...
static ___forceinline void draw_line(float *output, int x0, int y0, int x1, int y1, int n)
{
   int dy = y1 - y0;
   int adx = x1 - x0;
   int ady = abs(dy);
...
// they could be pushed back up but eh. ___forceinline showed no change;
...
static ___forceinline void iter_54(float *z)
{

...
static const char* findSubChunk(const char* data, size_t len, const char* id, size_t* size)
{
...
const char* wav_strerror(int err)
{
    switch(err)
    {
        ...
        case WAV_EFAILURE:
            return "failure";
        case WAV_EBADHEADER:
            return "bad header data";
        case WAV_EBADFMT:
            return "bad fmt data";
        case WAV_ENOFMT:
            return "missing 'fmt' subchunk";
        case WAV_ENODATA:
            return "missing 'data' subchunk";
        case WAV_ENOSUPPORT:
